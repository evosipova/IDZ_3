...
// Глобальный сервер сокет для обновлений
int updates_server_fd = -1;
struct sockaddr_in updates_addr;

void start_updates_server(char* ip_address, int port) {
  updates_server_fd = socket(AF_INET, SOCK_STREAM, 0);
  if (updates_server_fd == -1) {
    perror("socket");
    exit(1);
  }

  memset(&updates_addr, 0, sizeof(struct sockaddr_in));

  updates_addr.sin_family = AF_INET;
  updates_addr.sin_port = htons(port);
  updates_addr.sin_addr.s_addr = inet_addr(ip_address);

  if (bind(updates_server_fd, (struct sockaddr *)&updates_addr, sizeof(struct sockaddr_in)) == -1) {
    perror("bind");
    exit(1);
  }

  if (listen(updates_server_fd, 5) == -1) {
    perror("listen");
    exit(1);
  }
}
...
void update_hotel_status_file(char* status) {
  ...
  while (1) {
    int client_fd = accept(updates_server_fd, NULL, NULL);
    if (client_fd == -1) {
      perror("accept");
      break;
    }

    if (write(client_fd, status, strlen(status)) == -1) {
      perror("write");
      close(client_fd);
      break;
    }
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    fprintf(stderr, "Usage: %s <ip> <port> <updates_port>\n", argv[0]);
    exit(1);
  }

  char* ip_address = argv[1];
  int port = atoi(argv[2]);
  int updates_port = atoi(argv[3]);

  init_hotel();

  start_updates_server(ip_address, updates_port);
